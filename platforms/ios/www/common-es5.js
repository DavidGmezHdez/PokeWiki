  ...
  function srcAppServicesDatabaseServiceTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var DatabaseService = /*#__PURE__*/function () {
      function DatabaseService(plt, sqlitePorter, sqlite, http) {
        ...
        this.pokemons = new rxjs__WEBPACK_IMPORTED_MODULE_6__["BehaviorSubject"]([]);
      ...
      _createClass(DatabaseService, [{
        key: "crearDatabase",
        value: function crearDatabase() {
          var _this2 = this;

          this.http.get('assets/bd.sql', {
            responseType: 'text'
        ...
        value: function loadPokemons() {
          ...
          return this.database.executeSql('SELECT * FROM pokemon', []).then(function (data) {
            var pokemons = [];
            ...
            _this3.pokemons.next(pokemons);
          });
        }
      }, {
        key: "addPokemon",
        value: function addPokemon(name, tipo1, tipo2, porcentaje, img) {
          var _this4 = this;

          var data = [name, tipo1, tipo2, porcentaje, img];
          return this.database.executeSql('INSERT INTO pokemon (nombre,tipo1,tipo2,usePercent,img) VALUES (?,?,?,?,?)', data).then(function (data) {
            _this4.loadPokemons();
          });
        }
      }, {
        ...
        value: function getPokemon(id) {
          return this.database.executeSql('SELECT * FROM pokemon WHERE nombre = ?', [id]).then(function (data) {
            return {
        ...
        value: function deletePokemon(id) {
          ...
          return this.database.executeSql('DELETE FROM pokemon WHERE nombre = ?', [id]).then(function (_) {
            _this5.loadPokemons();
          });
        }
      }, {
        key: "updatePokemon",
        value: function updatePokemon(poke) {
          var _this6 = this;

          var data = [poke.name, poke.tipo1, poke.tipo2, poke.porcentaje, poke.img];
          return this.database.executeSql('UPDATE pokemon SET nombre = ?, tipo1 = ?, tipo2 = ?, usePercent = ?, img = ? WHERE idPoke = ${poke.id}', [data]).then(function (_) {
            _this6.loadPokemons();
          });
        }
      }]);
